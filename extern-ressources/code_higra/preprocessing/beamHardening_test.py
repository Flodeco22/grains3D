#!/usr/bin/env python
"""Adaptation of the beamHardening code by Gustavo.

Adapdations are :
- file gestion in command line;
- replacement of hard coded values with adaptative ones;
- no need to run script multiple times as thresholded image is genereted automatically;
- storage is made locally and is independant from user files tree structure.
"""
...
parser = argparse.ArgumentParser(
                    ...
                    epilog = "Written by Gustavo Pinzon")

parser.add_argument("greyscale", help = "The path to the greyscale tiff file.")
parser.add_argument("full", help="The path to the binary image.")

args = parser.parse_args()
grey_filepath = args.greyscale
full_filepath = args.full
...
imFull = tifffile.imread(full_filepath).astype(bool) 
# Greyscale image
imGrey = tifffile.imread(grey_filepath)
...
thresh = 0.75 * numpy.max(imGreyMask)
imBool = numpy.where(imGreyMask > thresh, 1, 0)
tifffile.imwrite(test+'_00_GreyBeam.tif', imGreyMask.astype(numpy.uint16))
...
tifffile.imwrite(test+'_00_BoolBeam.tif', imBool.astype(numpy.uint16))
...
voidMax = numpy.mean(peakVoid)
solidMax = numpy.mean(peakSolid)
...
# os.chdir(codeFolder)
# os.chdir(resFolder)
tifffile.imwrite(test+'_00.tif', imCorrected.astype(numpy.uint16))
