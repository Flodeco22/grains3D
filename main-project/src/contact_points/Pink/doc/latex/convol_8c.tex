\doxysection{convol.\+c File Reference}
...
{\bfseries{Usage\+:}} convol in.\+pgm kernel.\+pgm [mode] out.\+pgm
...
Naive convolution algorithm is in O($\vert$in$\vert$$\ast$$\vert$kernel$\vert$). For large kernels, use FFT version which is in O(n log n) where n = max($\vert$in$\vert$,$\vert$kernel$\vert$)
\end{DoxyWarning}
