\doxysection{toposhrinkgray.\+c File Reference}
...
{\bfseries{Usage\+:}} toposhrinkgray in.\+pgm connex t+min t+max t--min t--max \{0$\vert$a$\vert$d\} [inhibit] out.\+pgm

{\bfseries{Description\+:}} Topologically controled grayscale shrinking (one step).

The parameter $<$0$\vert$a$\vert$d$>$ indicates the action performed on the selected point p\+: \begin{DoxyItemize}
...
\begin{DoxyVerb}  For all points p, not in Y, and such that 
